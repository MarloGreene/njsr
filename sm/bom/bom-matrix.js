...
async function loadVerses() {
    try {
        ...
        if (!response.ok) {
            throw new Error('Could not load bom.txt');
        }
        const text = await response.text();
        verses = text.split('\n').filter(line => line.trim().length > 0);
        
        if (verses.length === 0) {
            throw new Error('No verses found in bom.txt');
        }
        ...
        loading.textContent = 'Error: Please add bom.txt file with verses (one per line, format: "Book Chapter:Verse Text")';
...
// Animation state
let isRunning = true;
let speed = parseInt(localStorage.getItem('bomMatrixSpeed')) || 50;
...
function draw() {
    ...
    columnData.forEach((col, i) => {
        ...
        ctx.fillStyle = `rgba(102, 153, 255, ${col.opacity})`;
...
speedSlider.addEventListener('input', (e) => {
    ...
    localStorage.setItem('bomMatrixSpeed', speed);
});
