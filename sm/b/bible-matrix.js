...
async function loadVerses() {
    try {
        ...
        if (!response.ok) {
            throw new Error('Could not load kjv.txt');
        }
        const text = await response.text();
        verses = text.split('\n').filter(line => line.trim().length > 0);
        
        if (verses.length === 0) {
            throw new Error('No verses found in kjv.txt');
        }
        ...
        loading.textContent = 'Error: Please add kjv.txt file with verses (one per line, format: "Book Chapter:Verse Text")';
...
// Animation state
let isRunning = true;
let speed = parseInt(localStorage.getItem('bibleMatrixSpeed')) || 50;
...
speedSlider.addEventListener('input', (e) => {
    ...
    localStorage.setItem('bibleMatrixSpeed', speed);
});
